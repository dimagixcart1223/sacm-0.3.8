	...
	if (IsActive()) {
		...
		if (m_byteTime) // If this player has private time
		{
			if (m_fGameTime - fLastSync >= 300.0f) // Sync to server every 5 minutes (no chance of lagging in there really)
			{
				...
				pNetGame->SendRPC(RPC_SetTimeEx, &bsPlayerTime, m_bytePlayerID, FALSE);
			}
		}
	}
...
void CPlayer::BroadcastSyncData()
{
	...
	{
		ONFOOT_SYNC_DATA ofSync = m_ofSync;
...
{
	SACMVEHICLE TrailerID = m_icSync.TrailerID;
	if (!TrailerID) return;
...
void CPlayer::StoreSpectatorFullSyncData(SPECTATOR_SYNC_DATA *pspSync)
{
	...
	if (m_byteState != PLAYER_STATE_SPECTATING) {
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerSpectating, &bsSend, m_bytePlayerID, TRUE);
	}
...
{
	SACMPLAYER playerid = m_bytePlayerID;
	RakNet::BitStream bsSend;
	...
	if (pNetGame->m_bLimitGlobalChatRadius) {
		...
		pNetGame->SendRPC(RPC_Chat, &bsSend, m_bytePlayerID, FALSE);
...
void CPlayer::HandleDeath(BYTE byteReason, SACMPLAYER byteWhoWasResponsible)
{
	...
	pNetGame->SendRPC(RPC_WorldPlayerDeath, &bsPlayerDeath, m_bytePlayerID, TRUE);
...
void CPlayer::Spawn()
{
	...
	if (m_bHasSpawnInfo) {
		...
		m_ofSync.byteHealth = 100;
		m_ofSync.byteArmour = 0;
		...
		pNetGame->GetPlayerPool()->InitSpawnsForPlayer(m_bytePlayerID);
	}
}
...
void CPlayer::SpawnForPlayer(SACMPLAYER forPlayerId)
{
	RakNet::BitStream bsPlayerSpawn;
	BYTE fightstyle = 0;

	bsPlayerSpawn.Write(m_bytePlayerID);
	bsPlayerSpawn.Write((char *)&m_SpawnInfo, sizeof(PLAYER_SPAWN_INFO));
	bsPlayerSpawn.Write(m_dwColor);
	bsPlayerSpawn.Write(m_byteFightingStyle);
	BOOL bVisible = TRUE;
	bsPlayerSpawn.Write(bVisible);

	pNetGame->SendRPC(RPC_WorldPlayerAdd, &bsPlayerSpawn, (forPlayerId != INVALID_PLAYER_ID) ? forPlayerId : m_bytePlayerID, (forPlayerId != INVALID_PLAYER_ID) ? false : true);
}

//----------------------------------------------------

void CPlayer::EnterVehicle(SACMVEHICLE VehicleID, BYTE bytePassenger)
{
	...
	pNetGame->SendRPC(RPC_EnterVehicle, &bsVehicle, playerid, TRUE);
}

//----------------------------------------------------

void CPlayer::ExitVehicle(SACMVEHICLE VehicleID)
{
	// Just send the notification to all the clients.
	RakNet::BitStream bsVehicle;
	SACMPLAYER playerid = m_bytePlayerID;
	...
	pNetGame->SendRPC(RPC_ExitVehicle, &bsVehicle, playerid, TRUE);
}
...
void CPlayer::SetPlayerColor(DWORD dwColor)
{
	...
	pNetGame->SendRPC(RPC_ScrSetPlayerColor, &bsColor, -1, TRUE);
}
...
void CPlayer::ToggleCheckpoint(BOOL bEnabled)
{
	...
	if (bEnabled)
	{
		...
		pNetGame->SendRPC(RPC_SetCheckpoint, &bsParams, m_bytePlayerID, FALSE);
	}
	else {
		pNetGame->SendRPC(RPC_DisableCheckpoint, &bsParams, m_bytePlayerID, FALSE);
	}
}
...
void CPlayer::ToggleRaceCheckpoint(BOOL bEnabled)
{
	...
	if (bEnabled)
	{
		...
		pNetGame->SendRPC(RPC_SetRaceCheckpoint, &bsParams, m_bytePlayerID, FALSE);
	}
	else {
		pNetGame->SendRPC(RPC_DisableRaceCheckpoint, &bsParams, m_bytePlayerID, FALSE);
	}
}
...
void CPlayer::SetClock(BYTE byteClock)
{
	...
	pNetGame->SendRPC(RPC_ToggleClock, &bsPlayerClock, m_bytePlayerID, FALSE);
}
