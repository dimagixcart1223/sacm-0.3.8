...
static cell AMX_NATIVE_CALL n_AddStaticVehicle(AMX *amx, cell *params)
{
	...
	SACMVEHICLE ret = pNetGame->GetVehiclePool()->New((int)params[1], &vecPos, amx_ctof(params[5]),
		(int)params[6], (int)params[7], 120000);

	// Add the model to the vehicle model array
	pNetGame->m_byteVehicleModels[params[1] - 400] = 1;
...
static cell AMX_NATIVE_CALL n_SetVehicleToRespawn(AMX *amx, cell *params)
{
	...
	CVehicle* pVehicle;
	pVehicle = pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[1]);

	if (pVehicle != NULL) {
		pVehicle->Respawn();
		RakNet::BitStream bsVehicle;
		bsVehicle.Write((SACMVEHICLE)params[1]);
		pNetGame->SendRPC(RPC_ScrRespawnVehicle, &bsVehicle, -1, TRUE);
		return 1;
	}
...
static cell AMX_NATIVE_CALL n_LinkVehicleToInterior(AMX *amx, cell *params)
{
	...
	if (pVehicle != NULL)
	{
		...
		bsData.Write((SACMVEHICLE)params[1]);
		bsData.Write((BYTE)params[2]);

		pNetGame->SendRPC(RPC_ScrLinkVehicle, &bsData, -1, TRUE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_AddVehicleComponent(AMX *amx, cell *params)
{
	...
	CVehicle* pVehicle = pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[1]);
	if (!pVehicle)
		return 0;

	RakNet::BitStream bsData;
	bsData.Write((SACMPLAYER)INVALID_PLAYER_ID);
	bsData.Write((int)EVENT_TYPE_CARCOMPONENT);
	bsData.Write((DWORD)params[1]);
	bsData.Write((DWORD)params[2]);
	bsData.Write((DWORD)0);

	pNetGame->SendRPC(RPC_ScmEvent, &bsData, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_RemoveVehicleComponent(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrRemoveComponent, &bsData, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_SetVehicleNumberPlate(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[1]))
	{
		char *szInput;
		...
		pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[1])->SetNumberPlate(szPlate);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_GetVehicleModel(AMX *amx, cell *params)
{
	...
	if (pVehiclePool && pVehiclePool->GetSlotState((SACMVEHICLE)params[1])) {
		return pVehiclePool->GetAt((SACMVEHICLE)params[1])->m_SpawnInfo.iVehicleType;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SetPlayerWorldBounds(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetWorldBounds, &bsBounds, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_Ban(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1])) {
		/*	RakPeerInterface* pRak = pRakServer;
			SACMPLAYER Player = params[1];

			in_addr in;
			in.s_addr = Player.binaryAddress;
			pNetGame->AddBan(pNetGame->GetPlayerPool()->GetPlayerName(params[1]), inet_ntoa(in), "INGAME BAN");
			pNetGame->KickPlayer(params[1]);*/
		return 1;
	}
	return 0;
...
static cell AMX_NATIVE_CALL n_BanEx(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1])) {
		/*		RakPeerInterface* pRak = pRakServer;
				SACMPLAYER Player = pRak->GetSystemAddressFromIndex(params[1]);
				...
				pNetGame->AddBan(pNetGame->GetPlayerPool()->GetPlayerName(params[1]), inet_ntoa(in), szReason);
				pNetGame->KickPlayer(params[1]);*/
		return 1;
	}
	return 0;
...
static cell AMX_NATIVE_CALL n_IsPlayerAdmin(AMX *amx, cell *params)
{
	...
	{
		return pPlayerPool->IsAdmin(params[1]);
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SetSpawnInfo(AMX *amx, cell *params)
{
	...
	if (pPlayer)
	{
		...
		pNetGame->SendRPC(RPC_ScrSetSpawnInfo, &bsData, params[1], FALSE);
		return 1;
	}
	else {
		return 0;
...
static cell AMX_NATIVE_CALL n_SpawnPlayer(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1]))
	{
		...
		pNetGame->SendRPC(RPC_RequestSpawn, &bsData, params[1], FALSE);
		return 1;
	}
	else {
		return 0;
...
static cell AMX_NATIVE_CALL n_ForceClassSelection(AMX *amx, cell *params)
{
	...
	{
		RakNet::BitStream bsData;
		pNetGame->SendRPC(RPC_ScrForceSpawnSelection, &bsData, params[1], FALSE);
		return 1;
	}
	else {
		return 0;
...
static cell AMX_NATIVE_CALL n_SetPlayerName(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		SACMPLAYER bytePlayerID = params[1];
		...
		if (byteSuccess != 0)
		{
			...
			pNetGame->SendRPC(RPC_ScrSetPlayerName, &bsData, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_SetPlayerSkin(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		if (pPlayer->m_bHasSpawnInfo)
		{
			...
			pNetGame->SendRPC(RPC_ScrSetPlayerSkin, &bsData, -1, TRUE);
		}
...
static cell AMX_NATIVE_CALL n_SetVehicleVirtualWorld(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[1]))
	{
		pNetGame->GetVehiclePool()->SetVehicleVirtualWorld((SACMVEHICLE)params[1], (BYTE)params[2]);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_GetVehicleVirtualWorld(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[1]))
	{
		return pNetGame->GetVehiclePool()->GetVehicleVirtualWorld((SACMVEHICLE)params[1]);
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_TogglePlayerSpectating(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1]))
	{
		...
		pNetGame->SendRPC(RPC_ScrTogglePlayerSpectating, &bsParams, params[1], FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_PlayerSpectateVehicle(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1])
		&& pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[2]))
	{
		CPlayer *pPlayer = pNetGame->GetPlayerPool()->GetAt(params[1]);
		pPlayer->m_SpectateID = (SACMVEHICLE)params[2];
		pPlayer->m_byteSpectateType = SPECTATE_TYPE_VEHICLE;

		RakNet::BitStream bsParams;
		bsParams.Write((SACMVEHICLE)params[2]); // vehicleid
		bsParams.Write((BYTE)params[3]); // mode

		pNetGame->SendRPC(RPC_ScrPlayerSpectateVehicle, &bsParams, params[1], FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_PlayerSpectatePlayer(AMX *amx, cell *params)
{
		...
		&& pNetGame->GetPlayerPool()->GetSlotState(params[2]))
	{
		...
		RakNet::BitStream bsParams;
		bsParams.Write((SACMPLAYER)params[2]); // playerid
		bsParams.Write((BYTE)params[3]); // mode

		pNetGame->SendRPC(RPC_ScrPlayerSpectatePlayer, &bsParams, params[1], FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SetPlayerTeam(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		RakNet::BitStream bsParams;
		bsParams.Write((SACMPLAYER)params[1]); // playerid
		bsParams.Write((BYTE)params[2]); // team id

		pNetGame->SendRPC(RPC_ScrSetPlayerTeam, &bsParams, -1, TRUE);
		return 1;
	}
	else {
		return 0;
...
static cell AMX_NATIVE_CALL n_SetPlayerPos(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerPos, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerPosFindZ(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerPosFindZ, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerHealth(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(BYTE(params[1])))
	{
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerHealth, &bsHealth, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_PutPlayerInVehicle(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1]) && pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[2]))
	{
		RakNet::BitStream bsParams;
		bsParams.Write((SACMVEHICLE)params[2]);	// vehicleid
		...
		if ((BYTE)params[3] == 0) { // driver
			pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[2])->SetDriver((BYTE)params[1]);
		}

		pNetGame->SendRPC(RPC_ScrPutPlayerInVehicle, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_RemovePlayerFromVehicle(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrRemovePlayerFromVehicle, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_CreateVehicle(AMX *amx, cell *params)
{
	...
	/*SACMVEHICLE VehicleID = pNetGame->GetVehiclePool()->New((int)params[1], &vecPos,
...
static cell AMX_NATIVE_CALL n_DestroyVehicle(AMX *amx, cell *params)
{
	...
	CVehiclePool*	pVehiclePool = pNetGame->GetVehiclePool();
	if (pVehiclePool->GetAt((SACMVEHICLE)params[1]))
	{
		pVehiclePool->Delete((SACMVEHICLE)params[1]);
		RakNet::BitStream bsParams;

		bsParams.Write((SACMVEHICLE)params[1]);

		pNetGame->SendRPC(RPC_VehicleDestroy, &bsParams, -1, TRUE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SetVehiclePos(AMX *amx, cell *params)
{
	...
	if (pVehicle)
	{
		if (pVehicle->GetDriver() != INVALID_ID)
		{
			RakNet::BitStream bsParams;
			bsParams.Write((SACMVEHICLE)params[1]);
			...
			pNetGame->SendRPC(RPC_ScrSetVehiclePos, &bsParams, pVehicle->GetDriver(), FALSE);
		}
...
// native SendPlayerMessageToPlayer(playerid, pRakServer->GetSystemAddressFromIndex(sender)id, const message[])
static cell AMX_NATIVE_CALL n_SendPlayerMessageToPlayer(AMX *amx, cell *params)
{
		...
		&& pNetGame->GetPlayerPool()->GetSlotState(params[2]))
	{
		...
		bsSend.Write((SACMPLAYER)params[2]);
		bsSend.Write(byteTextLen);
		bsSend.Write(szMessage, byteTextLen);
		pNetGame->SendRPC(RPC_Chat, &bsSend, params[1], FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SendPlayerMessageToAll(AMX *amx, cell *params)
{
	...
	if (pNetGame->GetPlayerPool()->GetSlotState(params[1]))
	{
		...
		RakNet::BitStream bsSend;
		bsSend.Write((SACMPLAYER)params[1]);
		bsSend.Write(byteTextLen);
		bsSend.Write(szMessage, byteTextLen);
		pNetGame->SendRPC(RPC_Chat, &bsSend, -1, TRUE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SendDeathMessage(AMX *amx, cell *params)
{
	...
	bsDM.Write((SACMPLAYER)params[1]);
	bsDM.Write((SACMPLAYER)params[2]);
	bsDM.Write((BYTE)params[3]);

	pNetGame->SendRPC(RPC_ScrDeathMessage, &bsDM, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_GameTextForAll(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrDisplayGameText, &bsParams, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_GameTextForPlayer(AMX *amx, cell *params)
{
	...
	bsParams.Write(iLength);
	bsParams.Write(szMessage, iLength);
	pNetGame->SendRPC(RPC_ScrDisplayGameText, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerInterior(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetInterior, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerSpecialAction(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetSpecialAction, &bsParams, params[1], FALSE);
	return 1;
}
...
static cell AMX_NATIVE_CALL n_SetPlayerCameraPos(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetCameraPos, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerCameraLookAt(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetCameraLookAt, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetCameraBehindPlayer(AMX *amx, cell *params)
{
	...
	if (!pNetGame->GetPlayerPool()->GetSlotState(params[1])) return 0;
	RakNet::BitStream bsParams;
	pNetGame->SendRPC(RPC_ScrSetCameraBehindPlayer, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_TogglePlayerControllable(AMX *amx, cell *params)
{
	...
	RakNet::BitStream bsParams;
	bsParams.Write((BYTE)params[2]);
	pNetGame->SendRPC(RPC_ScrTogglePlayerControllable, &bsParams, params[1], FALSE);
...
// native SetVehicleParametersForPlayer(vehicleid,pRakServer->GetSystemAddressFromIndex(playerid),objective,doorslocked)
static cell AMX_NATIVE_CALL n_SetVehicleParamsForPlayer(AMX *amx, cell *params)
{
	...
	if (!pNetGame->GetPlayerPool()->GetSlotState(params[2]) ||
		!pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[1])) return 0;
	RakNet::BitStream bsParams;
	//VehicleID = (SACMVEHICLE)params[1];
	...
	pNetGame->SendRPC(RPC_ScrVehicleParams, &bsParams, params[2], FALSE);
...
static cell AMX_NATIVE_CALL n_GivePlayerMoney(AMX *amx, cell *params)
{
	...
	if (pPool->GetSlotState(params[1])) {
		pPool->SetPlayerMoney(params[1], pPool->GetPlayerMoney(params[1]) + params[2]);

		pNetGame->SendRPC(RPC_ScrHaveSomeMoney, &bsMoney, params[1], FALSE);
		return 1;
	}
...
static cell AMX_NATIVE_CALL n_ResetPlayerMoney(AMX *amx, cell *params)
{
	...
	if (pPlayerPool->GetSlotState(bytePlayerID)) {
		...
		pNetGame->SendRPC(RPC_ScrResetMoney, &bsMoney, bytePlayerID, FALSE);
		return 1;
	}
...
static cell AMX_NATIVE_CALL n_SetPlayerAmmo(AMX *amx, cell *params)
{
	...
	if (pPlayerPool->GetSlotState(bytePlayerID))
	{
		...
		pNetGame->SendRPC(RPC_ScrSetWeaponAmmo, &bsAmmo, bytePlayerID, FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_SetPlayerFacingAngle(AMX *amx, cell *params)
{
	...
	bsFace.Write(fFace);
	pNetGame->SendRPC(RPC_ScrSetPlayerFacingAngle, &bsFace, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_ResetPlayerWeapons(AMX *amx, cell *params)
{
	...
	RakNet::BitStream bsData;
	pNetGame->SendRPC(RPC_ScrResetPlayerWeapons, &bsData, params[1], FALSE);
	return 1;
}
...
static cell AMX_NATIVE_CALL n_GivePlayerWeapon(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrGivePlayerWeapon, &bsData, params[1], FALSE);
	return 1;
}
...
static cell AMX_NATIVE_CALL n_SetVehicleZAngle(AMX *amx, cell *params)
{
	...
	if (pVehicle)
	{
		if (pVehicle->GetDriver() != INVALID_ID)
		{
			...
			pNetGame->SendRPC(RPC_ScrSetVehicleZAngle, &bsParams, pVehicle->GetDriver(), FALSE);
		}
		return 1;
	}
	else {
...
static cell AMX_NATIVE_CALL n_PlayerPlaySound(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrPlaySound, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_ShowPlayerNameTagForPlayer(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrShowNameTag, &bsParams, params[1], FALSE);
	return 1;
}
...
static cell AMX_NATIVE_CALL n_SetPlayerArmour(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetPlayerArmour, &bsArmour, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerMarkerForPlayer(AMX *amx, cell *params)
{
		...
		pNetGame->GetPlayerPool()->GetAt(params[2]))
	{
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerColor, &bsMarker, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerMapIcon(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetMapIcon, &bsIcon, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_RemovePlayerMapIcon(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrDisableMapIcon, &bsIcon, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerWeather(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_Weather, &bsWeather, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_GetVehicleHealth(AMX *amx, cell *params)
{
	...
	CVehicle* Vehicle = pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[1]);
...
static cell AMX_NATIVE_CALL n_AttachTrailerToVehicle(AMX *amx, cell *params)
{
	...
	if (pVehiclePool->GetAt((SACMVEHICLE)params[1]) && pVehiclePool->GetAt((SACMVEHICLE)params[2]))
	{
		RakNet::BitStream bsParams;
		bsParams.Write((SACMVEHICLE)params[1]);
		bsParams.Write((SACMVEHICLE)params[2]);
		pNetGame->SendRPC(RPC_ScrAttachTrailerToVehicle, &bsParams, -1, TRUE);
	}
	return 1;
}
...
static cell AMX_NATIVE_CALL n_DetachTrailerFromVehicle(AMX *amx, cell *params)
{
	...
	if (pVehiclePool->GetAt((SACMVEHICLE)params[1]) && pVehiclePool->GetAt((SACMVEHICLE)params[2]))
	{
		RakNet::BitStream bsParams;
		bsParams.Write((SACMVEHICLE)params[1]);
		pNetGame->SendRPC(RPC_ScrDetachTrailerFromVehicle, &bsParams, -1, TRUE);
	}
	return 1;
}
...
static cell AMX_NATIVE_CALL n_IsTrailerAttachedToVehicle(AMX *amx, cell *params)
{
	CHECK_PARAMS(1);
	CVehiclePool* pVehiclePool = pNetGame->GetVehiclePool();
	if (pVehiclePool->GetAt((SACMVEHICLE)params[1])->m_TrailerID != 0)
	{
		return 1;
	}
	return 0;
...
{
	CHECK_PARAMS(1);
	if (!pNetGame->GetVehiclePool()->GetSlotState((SACMVEHICLE)params[1])) return 0;
	return (cell)pNetGame->GetVehiclePool()->GetAt((SACMVEHICLE)params[1])->m_TrailerID;
}
...
static cell AMX_NATIVE_CALL n_SetObjectPos(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetObjectPos, &bsParams, -1, TRUE);
	CObjectPool *pObjectPool = pNetGame->GetObjectPool();
...
static cell AMX_NATIVE_CALL n_SetObjectRot(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetObjectRotation, &bsParams, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_DestroyObject(AMX *amx, cell *params)
{
	...
	if (pObjectPool->GetAt((BYTE)params[1]))
	{
		...
		pNetGame->SendRPC(RPC_ScrDestroyObject, &bsParams, -1, TRUE);
	}
...
static cell AMX_NATIVE_CALL n_MoveObject(AMX *amx, cell *params)
{
	...
	if (pObject)
	{
		...
		pNetGame->SendRPC(RPC_ScrMoveObject, &bsParams, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_StopObject(AMX *amx, cell *params)
{
	...
	if (pObject)
	{
		...
		pNetGame->SendRPC(RPC_ScrStopObject, &bsParams, -1, TRUE);
		//return *amx_ftoc(pObject->MoveTo(x, y, z, s));
		return 1;
	}
	return 0;
...
static cell AMX_NATIVE_CALL n_SetPlayerObjectPos(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetObjectPos, &bsParams, params[1], FALSE);
...
static cell AMX_NATIVE_CALL n_SetPlayerObjectRot(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrSetObjectRotation, &bsParams, params[1], TRUE);
...
static cell AMX_NATIVE_CALL n_DestroyPlayerObject(AMX *amx, cell *params)
{
	...
	if (pObjectPool->GetAtIndividual((BYTE)params[1], (BYTE)params[2]) && pObjectPool->DeleteForPlayer((BYTE)params[1], (BYTE)params[2]))
	{
		...
		pNetGame->SendRPC(RPC_ScrDestroyObject, &bsParams, params[1], FALSE);
	}
...
static cell AMX_NATIVE_CALL n_MovePlayerObject(AMX *amx, cell *params)
{
	...
	if (pObject)
	{
		...
		pNetGame->SendRPC(RPC_ScrMoveObject, &bsParams, bytePlayer, FALSE);
		//return *amx_ftoc(pObject->MoveTo(x, y, z, s));
		ret = pObject->MoveTo(x, y, z, s);
	}
	return (int)(ret * 1000.0f);
...
static cell AMX_NATIVE_CALL n_StopPlayerObject(AMX *amx, cell *params)
{
	...
	if (pObject)
	{
		...
		pNetGame->SendRPC(RPC_ScrStopObject, &bsParams, bytePlayer, FALSE);
		//return *amx_ftoc(pObject->MoveTo(x, y, z, s));
		return 1;
	}
	return 0;
...
static cell AMX_NATIVE_CALL n_CreateExplosion(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrCreateExplosion, &bsParams, -1, TRUE);
...
static cell AMX_NATIVE_CALL n_AttachObjectToPlayer(AMX *amx, cell *params)
{
		...
		pNetGame->GetPlayerPool()->GetAt(params[2]))
	{
		...
		pNetGame->SendRPC(RPC_ScrAttachObjectToPlayer, &bsParams, -1, TRUE);
	}
...
static cell AMX_NATIVE_CALL n_AttachPlayerObjectToPlayer(AMX *amx, cell *params)
{
		...
		pNetGame->GetPlayerPool()->GetAt(params[3]))
	{
		...
		pNetGame->SendRPC(RPC_ScrAttachObjectToPlayer, &bsParams, params[1], FALSE);
	}
...
static cell AMX_NATIVE_CALL n_SetPlayerWantedLevel(AMX *amx, cell *params)
{
	...
	if (pPlayer)
	{
		...
		pNetGame->SendRPC(RPC_ScrSetPlayerWantedLevel, &bsParams, params[1], FALSE);
		return 1;
	}
	return 0;
}
...
static cell AMX_NATIVE_CALL n_EnableStuntBonusForAll(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrEnableStuntBonus, &bsParams, -1, TRUE);
	return 1;
}
...
static cell AMX_NATIVE_CALL n_EnableStuntBonusForPlayer(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_ScrEnableStuntBonus, &bsParams, params[1], FALSE);
	return 1;
}
...
// native CreatePlayerPickup(pickupid,pRakServer->GetSystemAddressFromIndex(playerid),model,type,Float:PosX,Float:PosY,Float:PosZ)
static cell AMX_NATIVE_CALL n_CreatePlayerPickup(AMX *amx, cell *params)
{
	...
	pNetGame->SendRPC(RPC_Pickup, &bsPickup, params[2], FALSE);
...
// native DestroyPlayerPickup(pickupid,pRakServer->GetSystemAddressFromIndex(playerid))
static cell AMX_NATIVE_CALL n_DestroyPlayerPickup(AMX *amx, cell *params)
{
	...
	RakNet::BitStream bsPickup;
	bsPickup.Write(iPickupId);
	pNetGame->SendRPC(RPC_DestroyPickup, &bsPickup, params[2], FALSE);
