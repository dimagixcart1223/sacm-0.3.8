...
WORD CPlayerPed::GetKeys(WORD * lrAnalog, WORD * udAnalog)
{
	...
	*udAnalog = pInternalKeys->wKeys1[1]; // up/down analog
	...
	if(HasAmmoForCurrentWeapon() && pInternalKeys->wKeys1[4]) wRet |= 1; // secondary fire
...
void CPlayerPed::PutDirectlyInVehicle(int iVehicleID, int iSeat)
{
	...
	if(pNetGame) {
		...
		SACMVEHICLE TrainVehicleId = pVehiclePool->FindIDFromGtaPtr(pVehicle);
...
BYTE CPlayerPed::FindDeathReasonAndResponsiblePlayer(SACMPLAYER * nPlayer)
{	
	BYTE byteDeathReason;
	SACMPLAYER PlayerIDWhoKilled;
