...
float CVehicle::GetReportedHealth()
{
	if(m_pVehicle) return m_fReportedHealth;
	else return 0.0f;
}

//-----------------------------------------------------------

void CVehicle::SetReportedHealth(float fReportedHealth)
{
	if(m_pVehicle) {
		m_fReportedHealth = fReportedHealth;
	}
}
...
void CVehicle::RemoveMarkers()
{
	if(m_dwMarkerID) {
		ScriptCommand(&disable_marker, m_dwMarkerID);
		if(m_bSpecialMarkerEnabled) m_bSpecialMarkerEnabled = FALSE;
		m_dwMarkerID = 0;
	}
}
...
void CVehicle::GetReportedDamageStatus(DWORD * dwPanelDamage, DWORD * dwDoorDamage, BYTE * byteLightDamage)
{
	*dwPanelDamage = m_dwReportedPanelDamage;
	*dwDoorDamage = m_dwReportedDoorDamage;
	*byteLightDamage = m_byteReportedLightDamage;
}

//-----------------------------------------------------------

void CVehicle::SetReportedDamageStatus(DWORD dwPanelDamage, DWORD dwDoorDamage, BYTE byteLightDamage)
{
	m_dwReportedPanelDamage = dwPanelDamage;
	m_dwReportedDoorDamage = dwDoorDamage;
	m_byteReportedLightDamage = byteLightDamage;
}
...
CVehicle* CVehicle::GetTrailer()
{
	...
	if(pNetGame && dwTrailerGTAPtr) {
		...
		SACMVEHICLE TrailerID = (SACMVEHICLE)pVehiclePool->FindIDFromGtaPtr((VEHICLE_TYPE*)dwTrailerGTAPtr);
...
bool CVehicle::GetDeathNotified()
{
	return m_bDeathNotified;
}

//-----------------------------------------------------------

void CVehicle::SetDeathNotified(bool bNotified)
{
	m_bDeathNotified = bNotified;
}
