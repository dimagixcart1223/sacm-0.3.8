...
{
	for(SACMACTOR ActorID = 0; ActorID < MAX_ACTORS; ActorID++) {
...
BOOL CActorPool::New( SACMACTOR ActorID, int iSkin, VECTOR * vecPos, float fRotation, PCHAR szName )
{
...
BOOL CActorPool::Delete(SACMACTOR ActorID)
{
...
BOOL CActorPool::Spawn(SACMACTOR ActorID)
{	
...
SACMACTOR CActorPool::FindIDFromGtaPtr(PED_TYPE * pGtaPed)
{
	int x=1;
...
int CActorPool::FindNearestToLocalPlayerPed()
{
	...
	float fThisDistance;
	SACMACTOR ClosestSoFar=INVALID_ACTOR_ID;
