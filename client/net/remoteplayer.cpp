	...
	if(IsActive())
	{
		if(m_pPlayerPed)
		{
			...
			{	
				...
				// If the user has sent X, Y or Z offsets we need to 'attach' them to the car using these offsets
				if ( !IsSurfingOrTurretMode() )
				{
					// If the user hasn't sent these, they're obviously not on a car, so we just sync
				...
				//m_pPlayerPed->SetHealth(m_ofSync.byteHealth);
				//m_pPlayerPed->SetArmour(m_ofSync.byteArmour);
				...
				m_byteUpdateFromNetwork == UPDATE_TYPE_INCAR)
			{
				...
				m_pCurrentVehicle->SetReportedHealth(m_icSync.fCarHealth);
				...
				//m_pPlayerPed->SetHealth(m_icSync.bytePlayerHealth);
				//m_pPlayerPed->SetArmour(m_icSync.bytePlayerArmour);
				...
				m_byteUpdateFromNetwork == UPDATE_TYPE_PASSENGER)
			{
				...
				//m_pPlayerPed->SetHealth(m_psSync.bytePlayerHealth);
				//m_pPlayerPed->SetArmour(m_psSync.bytePlayerArmour);
...
void CRemotePlayer::UpdateSurfing()
{
	...
	if(pVehicle) {
		...
		matVehicle.pos.X = (m_ofSync.vecSurfSpeed.X + matVehicle.pos.X);
		matVehicle.pos.Y = (m_ofSync.vecSurfSpeed.Y + matVehicle.pos.Y);
		matVehicle.pos.Z = (m_ofSync.vecSurfSpeed.Z + matVehicle.pos.Z);
...
#ifdef _DEBUG
		pChatWindow->AddDebugMessage("Warning: Received surfing sync data from player %d with invalid vehicle id", m_PlayerID);
#endif
	}
...
{
	SACMVEHICLE UnocID = unocSync->VehicleID;
	...
	{
		pVehicle->SetHealth(unocSync->fHealth);
		pVehicle->UpdateDamageStatus(unocSync->dwPanelStatus, unocSync->dwDoorStatus, unocSync->byteLightStatus);
...
void CRemotePlayer::EnterVehicle(SACMVEHICLE VehicleID, BOOL bPassenger)
{
	CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
...
void CRemotePlayer::StateChange(BYTE byteNewState, BYTE byteOldState)
{
	...
	if(byteNewState == PLAYER_STATE_DRIVER && byteOldState == PLAYER_STATE_ONFOOT)
	{
		...
		CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
		SACMVEHICLE LocalVehicle=0xFFFF;
		MATRIX4X4 mat;
