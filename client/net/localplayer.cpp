...
void CLocalPlayer::ProcessUndrivenSync(SACMVEHICLE VehicleID, CVehicle *pVehicle, int iNumberOfPlayersInLocalRange)
{
	...
	if((dwThisTick - m_dwLastUndrivenUpdate) > (UINT)iSendRate) {
		...
		// or its health hasn't changed.
		DWORD dwPanelDamage;
		DWORD dwDoorDamage;
		BYTE byteLightDamage;
		pVehicle->GetReportedDamageStatus(&dwPanelDamage, &dwDoorDamage, &byteLightDamage);
		if ( (pVehicleType->pDriver && IN_VEHICLE(pVehicleType->pDriver) && 
			 (pVehicleType->pDriver->dwPedType == 0)) || 
			 pVehicle->IsStationary() && 
			 pVehicle->GetHealth() == pVehicle->GetReportedHealth() && 
			 pVehicle->GetPanelDamageStatus() == dwPanelDamage &&
			 pVehicle->GetDoorDamageStatus() == dwDoorDamage && 
			 pVehicle->GetLightDamageStatus() == byteLightDamage) { 
		...
		// Are we the closest player, and within 70 units of a moving nondriven or being damaged vehicle?
		if	( (iClosestPlayerID == pPlayerPool->GetLocalPlayerID()) &&
			( fSmallest <= UNOCCUPIED_SYNC_RADIUS ) ) {

	sync:
#ifdef _DEBUG
			pChatWindow->AddDebugMessage("UNDRIVEN [player: %s ID: %i sendrate: %i ms]", pNetGame->GetPlayerPool()->GetPlayerName(pNetGame->GetPlayerPool()->GetLocalPlayerID()) VehicleID, iSendRate);
#endif
			...
			UnSync.dwPanelStatus = pVehicle->GetPanelDamageStatus();
			UnSync.dwDoorStatus = pVehicle->GetDoorDamageStatus();
			UnSync.byteLightStatus = pVehicle->GetLightDamageStatus();
			...
			pVehicle->SetReportedHealth(pVehicle->GetHealth());
			pVehicle->SetReportedDamageStatus(pVehicle->GetPanelDamageStatus(), pVehicle->GetDoorDamageStatus(), pVehicle->GetLightDamageStatus());
...
void CLocalPlayer::HandlePassengerEntry()
{
	...
	if(pControls->wKeys1[8] && !pControls->wKeys2[8]) { // RECRUIT KEY JUST DOWN.
		...
		SACMVEHICLE ClosestVehicleID = pVehiclePool->FindNearestToLocalPlayerPed();
...
{	
	...
	SACMVEHICLE vehID = pNetGame->GetVehiclePool()->FindIDFromGtaPtr(Contact);

	if(vehID && vehID != INVALID_VEHICLE_ID) {

		CVehicle *pVehicle = pNetGame->GetVehiclePool()->GetAt(vehID);

		// NOTE: Enabling it for unoccupied vehicles
		// causes a load of problems (i.e. the headless char bug)
		if( pVehicle && pVehicle->IsOccupied() ) {
			// TODO: Remove unused surfing sync code
			MATRIX4X4 matPlayer;
			MATRIX4X4 matVehicle;
			VECTOR vecSpeed;
			VECTOR vecTurn;
			VECTOR vecSurfOffset;

			m_pPlayerPed->GetMatrix(&matPlayer);
			pVehicle->GetMatrix(&matVehicle);

			vecSurfOffset.X = (matPlayer.pos.X - matVehicle.pos.X);
			vecSurfOffset.Y = (matPlayer.pos.Y - matVehicle.pos.Y);
			vecSurfOffset.Z = (matPlayer.pos.Z - matVehicle.pos.Z);

			/*matPlayer.pos.X -= matVehicle.pos.X;
			matPlayer.pos.Y -= matVehicle.pos.Y;
			matPlayer.pos.Z -= matVehicle.pos.Z;*/
			...
			lr = pInternalKeys->wKeys1[0]; // left/right analog
			ud = pInternalKeys->wKeys1[1]; // up/down analog
			
			// if their analog keys/stick is 0 and they have surfing
			// offsets saved then keep them attached to the vehicle
			if(m_bSurfingMode && (!lr && !ud))
			{
				matVehicle.pos.X = (matVehicle.pos.X + m_vecSurfOffset.X);
				matVehicle.pos.Y = (matVehicle.pos.Y + m_vecSurfOffset.Y);
				matVehicle.pos.Z = (matVehicle.pos.Z + m_vecSurfOffset.Z);
				m_pPlayerPed->SetMatrix(matVehicle);
			}
			else
			{
				memcpy(&m_vecSurfOffset, &vecSurfOffset, sizeof(VECTOR));
			}
			...
			m_bSurfingMode = TRUE;
			m_surfVehicleID = vehID;
			// TEMP to prevent having to change a load of shit
			// When sync data stuff is cleaned up instead of removing
			// surf speed rename it to surf offset
			memcpy(&m_vecSurfSpeed, &m_vecSurfOffset, sizeof(VECTOR));
			//memcpy(&m_vecSurfSpeed, &vecSpeed, sizeof(VECTOR));
			memcpy(&m_vecSurfTurn, &vecTurn, sizeof(VECTOR));
			memcpy(&m_matSurf, &matPlayer, sizeof(MATRIX4X4));
			return;
		}
		/*if( pVehicle && pVehicle->IsOccupied() && 
			...
			if((lr || ud)) { // if(m_bSurfingMode == TRUE && (lr || ud)) {
...
void CLocalPlayer::SendOnFootFullSyncData()
{
	...
	if ( m_bSurfingMode ) {
		ofSync.bSurfing = TRUE;
		ofSync.surfVehicleID = m_surfVehicleID;
		ofSync.vecSurfSpeed = m_vecSurfSpeed;
		ofSync.vecSurfTurn = m_vecSurfTurn;
		ofSync.matSurf = m_matSurf;
	} else {
		ofSync.bSurfing = FALSE;
	}
...
void CLocalPlayer::SendWastedNotification()
{
	...
	pNetGame->SendRPC(RPC_Death,&bsPlayerDeath);
}
...
void CLocalPlayer::RequestClass(int iClass)
{
	...
	pNetGame->SendRPC(RPC_RequestClass,&bsSpawnRequest);
}
...
{
	RakNet::BitStream bsSpawnRequest;
	pNetGame->SendRPC(RPC_RequestSpawn,&bsSpawnRequest);
}
...
void CLocalPlayer::UpdateRemoteInterior(BYTE byteInterior)
{
	...
	pNetGame->SendRPC(RPC_SetInteriorId,&bsUpdateInterior);
}
...
BOOL CLocalPlayer::Spawn()
{
	...
	*(PBYTE)0x50AC22 = 0x00;
	...
	pNetGame->SendRPC(RPC_Spawn,&bsSendSpawn);
...
void CLocalPlayer::Say(PCHAR szText)
{
	...
	pNetGame->SendRPC(RPC_Chat,&bsSend);
}

//----------------------------------------------------------

void CLocalPlayer::SendEnterVehicleNotification(SACMVEHICLE VehicleID, BOOL bPassenger)
{
	RakNet::BitStream bsSend;
	...
	pNetGame->SendRPC(RPC_EnterVehicle,&bsSend);
...
void CLocalPlayer::SendExitVehicleNotification(SACMVEHICLE VehicleID)
{
	RakNet::BitStream bsSend;
	...
	{ 
		...
		if(!pVehicle->IsRCVehicle()) {
			bsSend.Write(VehicleID);
			pNetGame->SendRPC(RPC_ExitVehicle,&bsSend);
		}
	}
}
...
{
	CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
	SACMVEHICLE VehicleID = (SACMVEHICLE)pVehiclePool->FindIDFromGtaPtr(m_pPlayerPed->GetGtaVehicle());
...
void CLocalPlayer::ProcessSpectating()
{
	...
	else if (m_byteSpectateType == SPECTATE_TYPE_VEHICLE)
	{
		...
		if (pVehiclePool->GetSlotState((SACMVEHICLE)m_SpectateID)) {
			pVehicle = pVehiclePool->GetAt((SACMVEHICLE)m_SpectateID);
			if(pVehicle) {
...
void CLocalPlayer::SpectateVehicle(SACMVEHICLE VehicleID)
{
	CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
...
void CLocalPlayer::SpectatePlayer(SACMPLAYER playerId)
{
	CPlayerPool *pPlayerPool = pNetGame->GetPlayerPool();
...
void CLocalPlayer::ProcessVehicleDamageUpdates(SACMVEHICLE CurrentVehicle)
{
	...
	{			
			...
			// has had its damage model changed.
//#ifdef _DEBUG
...
//#endif
			...
			pNetGame->SendRPC(RPC_DamageVehicle,&bsVehicleDamageUpdate);
	}    
}
