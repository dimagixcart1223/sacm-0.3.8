...
BOOL CVehiclePool::New(VEHICLE_SPAWN_INFO *pNewVehicle)
{
	...
	{	
		...
		m_pVehicles[pNewVehicle->VehicleId]->SetReportedHealth(pNewVehicle->fHealth);
...
void CVehiclePool::LinkToInterior(SACMVEHICLE VehicleID, int iInterior)
{
...
void CVehiclePool::AssignSpecialParamsToVehicle(SACMVEHICLE VehicleID, BYTE byteObjective, BYTE byteDoorsLocked)
{
...
SACMVEHICLE CVehiclePool::FindIDFromGtaPtr(VEHICLE_TYPE * pGtaVehicle)
{
	int x=1;
...
void CVehiclePool::Process()
{
	...
	for(SACMVEHICLE x = 0; x != MAX_VEHICLES; x++)
	{
		if(GetSlotState(x) == TRUE)
		{
			...
			if(m_bIsActive[x])
			{
				if(pVehicle->GetDistanceFromLocalPlayerPed() >= 140.0f)
				{
					if(pVehicle->IsAdded() && !pVehicle->IsOccupied()/* && pLocalPlayer->m_LastVehicle != x*/)
					{
						// Stream it out
						pVehicle->RemoveMarkers();
						pVehicle->Remove();
					}
				}
				else
				{
					...
					if (pVehicle->GetHealth() == 0.0f && 
						!pVehicle->GetDeathNotified()) // || pVehicle->IsWrecked()) // It's dead
					{
					...
					if( pVehicle->GetVehicleSubtype() != VEHICLE_SUBTYPE_BOAT &&
						pVehicle->HasSunk() && 
						!pVehicle->GetDeathNotified() ) // Not boat and has sunk.
					{
...
void CVehiclePool::ProcessUnoccupiedSync(SACMVEHICLE VehicleID, CVehicle *pVehicle)
{
...
void CVehiclePool::NotifyVehicleDeath(SACMVEHICLE VehicleID)
{
	RakNet::BitStream bsDeath;
	bsDeath.Write(VehicleID);
	pNetGame->SendRPC(RPC_VehicleDestroyed, &bsDeath);
	GetAt(VehicleID)->SetDeathNotified(true); // Mark as notification sent
...
int CVehiclePool::FindNearestToLocalPlayerPed()
{
	...
	float fThisDistance;
	SACMVEHICLE ClosestSoFar=INVALID_VEHICLE_ID;
...
void CVehiclePool::NewWhenModelLoaded(VEHICLE_SPAWN_INFO *pNewVehicle)
{
	// find a free slot
	int x=0;
	while(x!=MAX_VEHICLE_WAITING_SLOTS) {
	...
	memcpy(&m_NewVehicleWaiting[x],pNewVehicle,sizeof(VEHICLE_SPAWN_INFO));
}
